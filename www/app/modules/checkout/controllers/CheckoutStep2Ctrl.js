...
angular
  ...
  .controller("CheckoutStep2Ctrl", CheckoutStep2Ctrl);

CheckoutStep2Ctrl.$inject = [
  "$scope",
  "$state",
  "$rootScope",
  "$stateParams",
...
function CheckoutStep2Ctrl(
  ...
  $scope.shippingMethodChanged = function() {
    ...
    var key = Object.keys(
      JSON.parse($scope.paymentAndShipping.shipping_method).quote
    )[0];
    var extended_url = "/shipping/method";
    var reqObj = {
      shipping_method: JSON.parse($scope.paymentAndShipping.shipping_method)
        .quote[key].code
    };
    var config = {
      headers: {
  ...
  function paymentMethodChanged() {
    ...
    console.log("payment method -->" + p_method);
  ...
  $scope.nextStep = function() {
    ...
    $state.go("checkout-step-3", {
      ...
      payment_and_shipping: $scope.paymentAndShipping
    });
  };
