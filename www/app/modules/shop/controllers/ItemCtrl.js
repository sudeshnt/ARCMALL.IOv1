...
function ItemCtrl(
  ...
  function addProductToCartAPI() {
    ...
    if ($scope.product.selected_options.length > 0) {
      for (var i in $scope.product.selected_options) {
        reqObj.option[$scope.product.selected_options[i].option_id] = parseInt(
          $scope.product.selected_options[i].option_value_id
        );
    ...
    httpService
      ...
      .then(function(response) {
        if (response.status === 200) {
          $rootScope.cartItemCount = !itemAvailability
            ? ($rootScope.cartItemCount += 1)
        ...
        if (response.status === 200) {
          ...
          $scope.groups = [
            ...
            },
            {
              ...
              show: false,
              content:
                "Returns accepted if the product is not as described or is damaged"
            },
            {
              name: $filter("translate")("BUYER_PROTECTION"),
              show: false,
              content:
                "To ensure that every buyer is protected when purchasing from Arcmall, we have the following refund options: \n" +
  ...
  function initRelatedProducts() {
    ...
    httpService
      ...
      .then(function(response) {
        if (response.status === 200) {
          $scope.relatedProducts = publicFunc.devideArray(
